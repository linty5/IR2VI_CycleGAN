...
class Generator(nn.Module):
    def __init__(self, opt):
        ...
        # Encoder
        self.E1 = Conv2dLayer(opt.in_channels, opt.start_channels, 7, 1, 3, pad_type = opt.pad, activation = opt.activ_g, norm = 'none')
        ...
        self.E4 = Conv2dLayer(opt.start_channels * 4, opt.start_channels * 8, 3, 2, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm)
        self.E5 = Conv2dLayer(opt.start_channels * 8, opt.start_channels * 16, 3, 2, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm)
        self.E6 = Conv2dLayer(opt.start_channels * 16, opt.start_channels * 16, 3, 2, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm)
        self.E7 = Conv2dLayer(opt.start_channels * 16, opt.start_channels * 16, 3, 2, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm)
        self.E8 = Conv2dLayer(opt.start_channels * 16, opt.start_channels * 16, 3, 2, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm)
        self.E9 = Conv2dLayer(opt.start_channels * 16, opt.start_channels * 16, 3, 2, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm)
        # Decoder
        self.D1 = TransposeConv2dLayer(opt.start_channels * 16, opt.start_channels * 16, 3, 1, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm, scale_factor = 2)
        self.D2 = TransposeConv2dLayer(opt.start_channels * 32, opt.start_channels * 16, 3, 1, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm, scale_factor = 2)
        self.D3 = TransposeConv2dLayer(opt.start_channels * 32, opt.start_channels * 16, 3, 1, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm, scale_factor = 2)
        self.D4 = TransposeConv2dLayer(opt.start_channels * 32, opt.start_channels * 16, 3, 1, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm, scale_factor = 2)
        self.D5 = TransposeConv2dLayer(opt.start_channels * 32, opt.start_channels * 8, 3, 1, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm, scale_factor = 2)
        self.D6 = TransposeConv2dLayer(opt.start_channels * 16, opt.start_channels * 4, 3, 1, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm, scale_factor = 2)
        self.D7 = TransposeConv2dLayer(opt.start_channels * 8, opt.start_channels * 2, 3, 1, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm, scale_factor = 2)
        self.D8 = TransposeConv2dLayer(opt.start_channels * 4, opt.start_channels, 3, 1, 1, pad_type = opt.pad, activation = opt.activ_g, norm = opt.norm, scale_factor = 2)
        self.D9 = Conv2dLayer(opt.start_channels * 2, opt.out_channels, 7, 1, 3, pad_type = opt.pad, activation = 'sigmoid', norm = 'none')
